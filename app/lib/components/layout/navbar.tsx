      ...
      <nav className="navbar has-background-secondary-level">
        ...
        <div className={`navbar-menu ${showMobileNav && "is-active"}`}>
          <div className="navbar-end">
            {user?.isAdmin && (
              <Link
                className={`navbar-item is-title medium is-uppercase is-tab px-0 mx-3 ${current_page == "/admin" ? "is-active" : ""}`}
                to="/admin"
              >
                Admin
              </Link>
            ...
            {user && (
              <Link
                className={`navbar-item is-title medium is-uppercase is-tab px-0 mx-3 ${current_page == "/" ? "is-active" : ""}`}
                to="/"
              >
                Accueil
              </Link>
            ...
            {user && (
              <Link
                className={`navbar-item is-title medium is-uppercase is-tab px-0 mx-3 ${current_page == "/info" ? "is-active" : ""}`}
                to="/info"
              >
                Infos
              </Link>
            ...
            {user && (
              <Link
                className={`navbar-item is-title medium is-uppercase is-tab px-0 mx-3 ${current_page == "/results" ? "is-active" : ""}`}
                to="/results"
              >
                Résultats
              </Link>
        ...
        <div id="waitingMatchesList" className="is-flex align-stretch gap-1">
          {playableMatches.map((match) => (
            <Link
              ...
              className="m-2 is-flex align-center gap-1 fade-on-mouse-out"
            >
              <span>{"//"}</span>
