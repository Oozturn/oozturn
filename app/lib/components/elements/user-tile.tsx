...
export function UserTileRectangle({
  userId,
  colorClass,
  height,
  maxLength,
  ...
  if (maxLength < height) maxLength = 2 * height
  const maxTeamLength = ((maxLength - height) * 1) / 3
  const maxUsernameLength = showTeam ? ((maxLength - height) * 2) / 3 : maxLength - height - 8
  return (
    <div
      key={user.id}
      className={`is-flex grow gap-1 align-center justify-stretch is-unselectable ${colorClass ? colorClass : ""}`}
    ...
    >
      ...
      {isShiny && (
        <div className="is-flex has-text-primary-accent">
          <ShinySVG />
        </div>
      )}
      <div style={{ maxWidth: maxUsernameLength + "px", overflow: "hidden", textOverflow: "ellipsis" }}>
        {isForfeit ? <s>{user.username}</s> : user.username}
      </div>
      {showTeam && user.team && (
        <div
          className="grow no-basis fade-text"
          style={{ maxWidth: maxTeamLength + "px", overflow: "hidden", textOverflow: "ellipsis", textWrap: "nowrap" }}
        >
          [{user.team}]
        </div>
      )}
...
export function FakeUserTileRectangle({
  userName,
  initial,
  teamName,
  colorClass,
  ...
  return (
    <div
      className={`is-flex gap-1 align-center justify-stretch is-unselectable ${colorClass ? colorClass : ""}`}
      style={{ maxWidth: maxLength, textAlign: "initial" }}
    >
...
export function UserTileUsersPage({ userId, tournaments, points, leaderboardPlace }: UserTileUsersPageProps) {
  ...
  const title = userStats
    ? "Tournois gagnés : " +
      String(userStats.wonTournaments) +
      "\nMeilleure position : " +
      String(userStats.bestTournamentPosition) +
      "\nPoints au classement global : " +
      String(userStats.globalTournamentPoints) +
      "\nPoints marqués/encaissés (normalisé) : " +
      String(userStats.pointsRatio.toFixed(2)) +
      "\nMatchs joués : " +
      String(userStats.playedMatches)
    : undefined

  return (
    <div
      key={user.id}
      className="is-flex-col no-basis grow gap-2 p-3 align-stretch is-unselectable has-background-secondary-level"
      ...
      <div className="is-flex-col grow gap-1 justify-center" style={{ overflow: "hidden", textOverflow: "ellipsis" }}>
        ...
        <div className="is-flex-col align-stretch grow justify-center">
          ...
          <div className="is-flex">
            ...
            <div className="">
              <sup>{["ère", "nde"][leaderboardPlace - 1] ?? "ème"}</sup> place
            </div>
          </div>
        </div>
      </div>
