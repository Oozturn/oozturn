...
export function UsersList({ admin = false }: { admin?: boolean }) {
  ...
  return (
    <div className="has-background-secondary-level usersList is-full-height is-flex-col pr-2 p-4 gap-3">
      <div className="is-flex justify-space-between align-center">
        ...
        {admin && (
          <CustomButton
            ...
            callback={() => setShowAddUsers(true)}
            contentItems={[SmallCrossSVG(), "Add users"]}
          />
        )}
      </div>
      <CustomModalBinary
        ...
        {users &&
          users
            ...
            .map((user) => (
              <div
                key={user.id}
                className={`userTile is-flex-col is-clickable ${activeUser == user.id ? "is-active" : ""}`}
                onMouseEnter={() => setHooveredUser(user.id)}
                onMouseLeave={() => setHooveredUser("")}
              >
                <div className="is-flex justify-space-between align-center">
                  <div
                    className="is-flex is-clickable grow"
                    {...clickorkey(() => setActiveUser(activeUser == user.id ? "" : user.id))}
                  >
                    <UserTileRectangle userId={user.id} height={40} />
                  </div>
                  {admin && (
                    <SquareButton
                      contentItems={[MoreSVG()]}
                      height={40}
                      show={hooveredUser == user.id}
                      callback={(e: TriggerEvent) => {
                        showMenu({
                          id: "usersMenu",
                          event: e,
                          props: {
                ...
                <div
                  className="userTooltip is-flex pl-3"
                  {...clickorkey(() => setActiveUser(activeUser == user.id ? "" : user.id))}
                >
      ...
      <Menu id="usersMenu" animation="slide">
        <Item id="renameUser" onClick={handleMenuItemClick}>
          Renommer
        </Item>
        {settings.security.authentication && (
          <Item id="resetPassword" onClick={handleMenuItemClick}>
            Réinitialiser le mot de passe
          </Item>
        )}
        <Item id="deleteUser" onClick={handleMenuItemClick}>
          Supprimer
        </Item>
      </Menu>
    </div>
