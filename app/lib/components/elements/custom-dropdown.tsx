...
export default function Dropdown({ trigger, items, direction, align, id }: DropdownInterface) {
    ...
    return <div className="is-relative is-clickable">
        <div id={id}>
            {trigger}
        </div>
        <div className="is-flex-direction-row has-background-primary-level gap-1 p-1" style={{
            display: show ? 'flex' : 'none', position: "absolute", minWidth: "100%", zIndex: 20,
            ...(align == 'left' ? { left: 0 } : align == 'right' ? { right: 0 } : { right: "50%" }),
