...
const secureStorage = createCookieSessionStorage({
...
async function getSession(request: Request) {
  return await secureStorage.getSession(request.headers.get("Cookie"))
}
...
export async function isUserLoggedIn(request: Request) {
  ...
  if (!(process.env.UNSAFE_NO_AUTHENTICATION === "true")) {
    return session.has("userId") && session.get("auth") === "password"
  }
