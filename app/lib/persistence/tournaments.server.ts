...
subscribeObjectManager("tournaments", {
  onRestore: () => {
    ...
    if (fs.existsSync(tournamentsFilePath)) {
      ...
      global.tournaments = (
        JSON.parse(fs.readFileSync(tournamentsFilePath, "utf-8"), scoresReviver) as TournamentStorage[]
      ).map((ts) => TournamentEngine.fromStorage(ts))
  ...
  onStore: () => {
    writeSafe(
      tournamentsFilePath,
      JSON.stringify(
        global.tournaments.map((tournament) => tournament.getStorage()),
        null,
        2
      )
    )
...
export function newTournament(
  ...
  if (global.tournaments.find((t) => t.getId() == tournamentId))
