...
subscribeObjectManager("lan", {
  onRestore: () => {
    ...
    if (fs.existsSync(lanFilePath)) {
      ...
      global.lan = {
        ...defaultLan,
        ...JSON.parse(fs.readFileSync(lanFilePath, "utf-8"))
      }
    } else {
...
export function updateLan(partialLan: Partial<Lan>) {
  ...
  if (partialLan.weightTeamsResults != undefined || partialLan.showPartialResults != undefined) {
    invalidateStats()
  }
  EventUpdateLan()
...
export function updateAchievements(partialAchievements: Partial<Achievement>[]) {
  partialAchievements.forEach((partialAchievement) => {
    ...
    if (achievementIndex != -1) {
      global.lan.achievements[achievementIndex] = {
        ...global.lan.achievements[achievementIndex],
        ...partialAchievement
      }
    } else {
