...
export abstract class Tournament {
  ...
  protected idToString(id: Id) {
    return id + "" === "[object Object]" ? "S" + id.s + " R" + id.r + " M" + id.m : id + ""
  }
...
export const matchTieCompute = function (
  zipSlice: [number, number][],
  startIdx: number,
  callback: (p: number, pos: number) => void
) {
  let pos = startIdx,
    ties = 0,
    scr = -Infinity
...
export const resTieCompute = function <M, R>(
  resAry: R[],
  startPos: number,
  cb: (r: R, pos: number) => void,
  metric: (r: R) => M
) {
  let pos = startPos,
    ties = 0,
