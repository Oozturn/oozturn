...
const elimination = function (size: number, p: number, last: number, isLong: boolean) {
  const matches: Match[] = []
  ...
  // blank WB rounds
  for (let r = 2; r <= p; r += 1) {
    for (let g = 1; g <= Math.pow(2, p - r); g += 1) {
  ...
  // blank LB rounds
  if (last >= LB) {
    for (let r = 1; r <= 2 * p - 2; r += 1) {
      // number of matches halves every odd round in losers bracket
      for (let g = 1; g <= Math.pow(2, p - 1 - Math.floor((r + 1) / 2)); g += 1) {
...
const seeds = function (i: number, p: number): [number, number] {
  const even = evenSeed(i, p)
...
const lbPos = function (p: number, maxr: number) {
...
const wbPos = function (p: number, maxr: number) {
...
const mixLbGames = function (p: number, round: number, game: number) {
  ...
  const partitioned = even(Math.floor((round + 1) / 2))
...
export interface DuelOpts extends TournamentOpts {
  short?: boolean
  last?: number
  downMix?: boolean
}
...
export class Duel extends Tournament {
  ...
  constructor(numPlayers: number, opts: DuelOpts) {
    ...
    //create matches
    const p = Math.ceil(Math.log(numPlayers) / Math.log(2))
  ...
  protected progress(match: Match): void {
    ...
    const dres = this.inserter(this.down(match.id), l)
  ...
  protected early(): boolean {
    const gf1 = this.matches[this.matches.length - 2]
  ...
  protected stats(res: Result[], g: Match): Result[] {
    ...
    if (g.p.indexOf(WO) < 0 && g.m) {
      ...
      const isConclusiveLbGf = isLbGfs && (g.id.r === 2 * p || !isLong || wbWinnerWon)
