...
export const equality =
  <T>(prop: string) =>
  (x: { [x: string]: T }, y: { [x: string]: T }) =>
    x[prop] === y[prop]
export const equalityBy =
  <T>(costFn: (input: T) => any) =>
  (x: T, y: T) =>
    costFn(x) === costFn(y)
export const compare = (order?: Order) => (x: number, y: number) => (order || 1) * (x - y)
export const compareBy = (costFn: (arg0: any) => number, order: Order) => (x: any, y: any) =>
  (order || 1) * (costFn(x) - costFn(y))
export const comparing = (prop: string, order: Order) => (x: { [x: string]: number }, y: { [x: string]: number }) =>
  (order || 1) * (x[prop] - y[prop])
...
export const maximumBy = (cmpFn: (arg0: any, arg1: any) => number, xs: any[]) =>
  xs.reduce((max: any, x: any) => (cmpFn(x, max) > 0 ? x : max), xs[0])
export const minimumBy = (cmpFn: (arg0: any, arg1: any) => number, xs: any[]) =>
  xs.reduce((min: any, x: any) => (cmpFn(x, min) < 0 ? x : min), xs[0])
