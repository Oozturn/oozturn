...
export class GroupStage extends Tournament {
  ...
  protected sort(resAry: Result[]): Result[] {
    ...
    resultsByGroup(resAry, this.numGroups).forEach((g) => {
      // g sorted as res is
  ...
  private compareResults = (x: Result, y: Result) => {
    ...
    return (
      y.pts / y.matches - x.pts / x.matches ||
      yScore / y.matches - xScore / x.matches ||
      y.for! / y.matches - x.for! / x.matches ||
      x.seed - y.seed
    )
  }
