...
export function updatePlayableMatches() {
  ...
  global.playableMatches = tournaments
    .flatMap((tournament) =>
      tournament
        .getMatches()
        ...
        .map((match) => {
          ...
          const tournamentForfeits = tournament
            .getPlayers()
            .filter((player) => player.isForfeit)
            .map((player) => player.userId)
          const concerned = match.opponents
            .flatMap((opponent) =>
              opponentsAreTeams ? tournament.getTeams().filter((team) => team.name == opponent)[0]?.members : opponent
            )
