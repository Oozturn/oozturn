...
function getAchievementUser(achievement: AchievementType) {
  ...
  switch (achievement) {
    case AchievementType.tournamentWinner:
      userStats = getStats()
        .usersStats.sort((a, b) => b.wonTournaments - a.wonTournaments)
        .at(0)
    ...
    case AchievementType.efficientWinner:
      userStats = getStats()
        .usersStats.sort((a, b) => b.wonTournaments / b.playedTournaments - a.wonTournaments / a.playedTournaments)
        .at(0)
      if (!userStats?.wonTournaments) break
      return {
        userId: userStats.userId,
        value: userStats.wonTournaments / userStats.playedTournaments
      }
    case AchievementType.neverFirst:
      userStats = getStats()
        .usersStats.sort((a, b) => b.secondPlaces - a.secondPlaces)
        .at(0)
    ...
    case AchievementType.worstPlayerEver:
      userStats = getStats()
        .usersStats.sort((a, b) => a.globalTournamentPoints - b.globalTournamentPoints)
        .at(0)
      if (!userStats) break
      return {
        userId: userStats.userId,
        value: userStats.globalTournamentPoints
      }
    case AchievementType.chiefWinner:
      userStats = getStats()
        .usersStats.sort((a, b) => b.pointsRatio - a.pointsRatio)
        .at(0)
      if (!userStats?.pointsRatio) break
      return { userId: userStats.userId, value: userStats.pointsRatio }
    case AchievementType.chiefLooser:
      userStats = getStats()
        .usersStats.sort((a, b) => a.pointsRatio - b.pointsRatio)
        .at(0)
      if (!userStats) break
      return { userId: userStats.userId, value: userStats.pointsRatio }
    case AchievementType.compulsivePlayer:
      userStats = getStats()
        .usersStats.sort((a, b) => b.playedMatches - a.playedMatches)
        .at(0)
      if (!userStats?.playedMatches) break
      return { userId: userStats.userId, value: userStats.playedMatches }
    case AchievementType.david:
      userStats = getStats()
        .usersStats.sort((a, b) => b.winsAgainstBetterSeed - a.winsAgainstBetterSeed)
        .at(0)
      if (!userStats?.winsAgainstBetterSeed) break
      return {
        userId: userStats.userId,
        value: userStats.winsAgainstBetterSeed
      }
    case AchievementType.underPressure:
      userStats = getStats()
        .usersStats.sort((a, b) => b.secondChances - a.secondChances)
        .at(0)
    ...
    case AchievementType.tryHarder:
      userStats = getStats()
        .usersStats.sort((a, b) => b.hardVictories - a.hardVictories)
        .at(0)
...
export function getAchievements(): Achievement[] {
  ...
  return Object.values(AchievementType).map((achievementType) => {
    ...
    return {
      ...
      description:
        lanAchievements.find((a) => a.type == achievementType)?.description ||
        AchievementDecriptors.get(achievementType) ||
        "",
      userId: achievementUser.userId,
      value: achievementUser.value,
      ...(AchievementValueDecriptor.get(achievementType) || {
        valueDescription: "",
        valueUseBest: true
      })
    }
