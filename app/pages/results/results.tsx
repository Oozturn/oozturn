...
function ResultTile({ place, user, teamName, stats, showInfo }: ResultPlayerTileProps) {
    ...
    return (
        <div className={`resultTile is-relative is-unselectable is-flex-col is-clipped has-background-primary-level pr-1 no-shrink ${teamName ? 'is-clickable teamResult' : ''}`} {...clickorkey(() => setActiveResult(showInfo ? "" : teamName || user?.username || ""))}>
            <div className="is-flex-row align-center gap-2" style={{
                ...
                <div className='place justify-end' style={{
                    zIndex: 2,
                    width: [teamName ? 95 : 86, 60, 35][place - 1] || "2rem",
                    textAlign: place < 4 ? "center" : "right",
                    fontStyle: "italic"
                }}>{place == 1 ? '1ER' : place < 4 ? place + 'E' : place + '.'}</div>
                <div className='opponent is-clipped is-flex grow no-basis' style={{
                    ...
                    {user && user.team && <div className='team fade-text' style={{ fontSize: 20 }}>[{user.team}]</div>}
                </div>
            ...
            {teamName &&
                <div className={`is-flex-row has-background-primary-level fade-text ${showInfo ? "showInfo" : ""}`} style={{ height: showInfo ? "unset" : 0 }}>
                    <div className="mr-2 mt-1" style={{
                        marginLeft: [99, 62, 30][place - 1] || 30,
                        backgroundColor: ["var(--accent-primary-color)", "var(--accent-secondary-color)", "var(--grey)"][place - 1] || "var(--text-color-70)",
                        paddingLeft: 1
                    }}></div>
                    <div className="">
