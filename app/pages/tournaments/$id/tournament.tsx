...
export default function TournamentPage() {
    ...
    const canAddPlayers = tournament.bracketSettings[0].type == BracketType.Duel || (tournament.players.length < GetFFAMaxPlayers(tournament.bracketSettings[0].sizes || [], tournament.bracketSettings[0].advancers || []) * (tournament.bracketSettings[0].useTeams ? tournament.bracketSettings[0].teamsMaxSize || 1 : 1))
...
function TournamentCommands() {
    ...
    return <div className='is-flex justify-end gap-3'>
        ...
        <CustomModalBinary show={showConfirmStart} onHide={() => setShowConfirmStart(false)} content={<>Es-tu sûr de vouloir démarrer ce tournoi ? <br/>{tournament.bracketSettings[0].useTeams ? "Les équipes vides et les joueurs sans équipes seront retirés du tournoi." : ""}</>} cancelButton={true} onConfirm={startTournament} />
        <CustomModalBinary show={showConfirmStop} onHide={() => setShowConfirmStop(false)} content={`Es-tu sûr de vouloir redémarrer ce tournoi ? Tu pourras éditer ${tournament.bracketSettings[0].useTeams ? "les équipes et " : ""}les inscriptions, mais toute sa progression sera perdue !`} cancelButton={true} onConfirm={stopTournament} />
    </div>
}