...
import {
  ...
  balanceTeams,
  randomizePlayersOnTeams,
  cancelTournament,
  startTournament,
  scoreMatch,
  stopTournament,
  toggleForfeitPlayerForTournament,
  validateTournament,
  togglePauseTournament
} from "./tournament.queries.server"
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  return [{ title: data?.lanName + " - Tournoi " + data?.tournament.properties.name }]
}
  ...
  try {
    switch (intent) {
      ...
      case TournamentManagementIntents.ADD_PLAYERS:
        ;(jsonData.userIds as string[]).forEach((uid) => addPlayerToTournament(jsonData.tournamentId as string, uid))
        break
      ...
      case MatchesIntents.SCORE:
        scoreMatch(
          jsonData.tournamentId as string,
          jsonData.matchID as string,
          jsonData.opponent as string,
          jsonData.score as number | null,
          (await getUserFromRequest(request)) as User
        )
        return json({
          error: null,
          type: MatchesIntents.SCORE,
          matchID: jsonData.matchID as string,
          opponent: jsonData.opponent as string
        })
      default:
        break
    }
    ...
    if (intent == MatchesIntents.SCORE) {
      return json({
        error: "Error while scoring match",
        type: MatchesIntents.SCORE,
        matchID: jsonData.matchID as string,
        opponent: jsonData.opponent as string
      })
...
export default function TournamentPage() {
  ...
  const canAddPlayers = (function () {
    ...
    const maxPlayers = GetFFAMaxPlayers(
      tournament.bracketSettings[0].sizes || [],
      tournament.bracketSettings[0].advancers || []
    )
    if (
      tournament.players.length <
      maxPlayers * (tournament.settings.useTeams ? tournament.settings.teamsMaxSize || 1 : 1)
    )
      return true
    return false
  })()
          ...
          <div className="has-background-secondary-level is-flex-row grow p-3 gap-6">
            ...
            {[TournamentStatus.Open, TournamentStatus.Balancing].includes(tournament.status) ? (
              <div className="is-flex-col grow no-basis">
                {tournament.settings.useTeams ? <OpponentsListTeam /> : <OpponentsListSolo />}
                ...
                <div className="is-flex-row justify-space-between gap-3">
                  {user.isAdmin ? (
                    <>
                      <CustomButton
                        ...
                        tooltip={
                          tournament.status == TournamentStatus.Open
                            ? "Empêcher les joueurs d'interragir avec le tournoi, pour pouvoir les re-seeder"
                            : "Réouvrir le tournoi aux joueurs"
                        }
                        contentItems={[
                          tournament.status == TournamentStatus.Open ? LockSVG() : UnlockSVG(),
                          "Inscriptions"
                        ]}
                      ...
                      {/**  DEV ONLY  */}
                      {process.env.NODE_ENV === "development" && canAddPlayers && (
                        <CustomButton
                          callback={addFakePlayer}
                          contentItems={["Add player"]}
                          colorClass="has-background-primary"
                        />
                      )}
                      {/**  DEV ONLY  */}
                    </>
                  ...
                  <>
                    ...
                    {tournament.players.find((player) => player.userId == user.id) ? (
                      <CustomButton
                        ...
                        contentItems={[LeaveSVG(), "Quitter"]}
                        colorClass="has-background-secondary-accent"
                      />
                    ) : (
                      <CustomButton
                        callback={joinTournament}
                        active={canAddPlayers}
                        contentItems={[ParticipateSVG(), "Participer"]}
...
function PlayersSelector() {
  ...
  function showModal() {
    setPlayersList(
      users
        .filter((u) => !tournament.players.map((p) => p.userId).includes(u.id))
        .map((u) => {
          return { id: u.id, name: u.username, checked: false }
        })
    )
  ...
  return (
    <>
      <CustomButton callback={showModal} contentItems={["Add players"]} colorClass="has-background-primary-level" />
      <CustomModalBinary
        show={showAddPlayers}
            ...
            <div className="is-flex wrap gap-3 is-scrollable has-background-primary-level p-2">
              {playersList.map((player) => (
                <div
                  ...
                  className={`grow is-clickable p-0 pr-2 has-background-${
                    player.checked ? "secondary-accent" : "secondary-level"
                  } is-flex-row align-center`}
                >
...
function TournamentCommands() {
  ...
  const showFFbutton =
    ![TournamentStatus.Open, TournamentStatus.Balancing, TournamentStatus.Validating, TournamentStatus.Done].includes(
      tournament.status
    ) &&
    tournament.players.find((player) => player.userId == user.id) &&
    !isForfeit
  ...
  if ([TournamentStatus.Running, TournamentStatus.Paused].includes(tournament.status) && showFFbutton)
    items.push({
      content:
        tournament.status == TournamentStatus.Running ? [LockSVG(), "Verrouiller"] : [StartSVG(), "Déverrouiller"],
  ...
  return (
    <div className="is-flex justify-end gap-3">
      {showFFbutton && (
        <>
          <CustomButton
            callback={() => setShowConfirmForfeit(true)}
            contentItems={[ForfeitSVG(), "Abandonner"]}
            colorClass="has-background-primary-accent"
          />
          <CustomModalBinary
            show={showConfirmForfeit}
            onHide={() => setShowConfirmForfeit(false)}
            content={"Es-tu sûr de vouloir abandonner ?"}
            cancelButton={true}
            onConfirm={forfeit}
          />
        </>
      ...
      {user.isAdmin && (
        <>
          {[TournamentStatus.Open, TournamentStatus.Balancing].includes(tournament.status) && (
            <CustomButton
              callback={() => setShowConfirmStart(true)}
              contentItems={[StartSVG(), "Démarrer"]}
              colorClass="has-background-primary-accent"
            />
          )}
          {[TournamentStatus.Validating].includes(tournament.status) && (
            <CustomButton
              callback={() => setShowConfirmValidate(true)}
              contentItems={[ThumbUpSVG(), "Valider"]}
              colorClass="has-background-primary-accent"
            />
          )}
          {[TournamentStatus.Running, TournamentStatus.Paused].includes(tournament.status) && !showFFbutton && (
            <CustomButton
              callback={togglePause}
              contentItems={
                tournament.status == TournamentStatus.Running
                  ? [LockSVG(), "Verrouiller"]
                  : [StartSVG(), "Déverrouiller"]
              }
              colorClass="has-background-primary-accent"
            />
          )}
          <Dropdown
            trigger={<SquareButton contentItems={[MoreSVG()]} colorClass="has-background-primary-level" />}
          ...
          <CustomModalBinary
            ...
            content={"Es-tu sûr de vouloir annuler ce tournoi ?"}
            cancelButton={true}
            onConfirm={cancelTournament}
          />
          <CustomModalBinary
            show={showConfirmStart}
            onHide={() => setShowConfirmStart(false)}
            content={
              <>
                Es-tu sûr de vouloir démarrer ce tournoi ?{" "}
                {tournament.settings.useTeams ? (
                  <>
                    <br />
                    Les équipes vides et les joueurs sans équipes seront retirés du tournoi.
                  </>
                ) : (
                  ""
                )}
          ...
          <CustomModalBinary
            ...
            onHide={() => setShowConfirmStop(false)}
            content={
              <>
                Es-tu sûr de vouloir démarrer ce tournoi ?<br />
                Tu pourras éditer {tournament.settings.useTeams ? "les équipes et " : ""}les inscriptions, mais toute la
                progression sera perdue !
              </>
            }
            cancelButton={true}
            onConfirm={stopTournament}
          />
          <CustomModalBinary
            show={showConfirmValidate}
            onHide={() => setShowConfirmValidate(false)}
            content={<>Es-tu sûr de vouloir valider {tournament.bracketsCount == 2 ? "cette phase" : "ce tournoi"} ?</>}
            cancelButton={true}
            onConfirm={validateTournament}
          />
        </>
      )}
    </div>
