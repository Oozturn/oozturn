      ...
      <div className="is-flex-col grow gap-2">
        ...
        {tournament.bracketsCount == 1 ?
          <div className="is-flex gap-3 wrap">
            ...
            <div>
              {tournament.bracketSettings[0].type} {tournament.settings.useTeams ? "par équipes" : "en solo"}
            </div>
          </div>
        : <>
            <div className="is-flex gap-3 wrap">
              ...
              <div>
                {tournament.bracketSettings[0].type} {tournament.settings.useTeams ? "par équipes" : "en solo"}{" "}
                {tournament.bracketSettings[1].size ? "" : "(Classement)"}
              </div>
            </div>
            <div className="is-flex gap-3 wrap">
              <div className="has-text-right is-one-quarter">Finale :</div>
              <div>
                {tournament.bracketSettings[1].type} {tournament.settings.useTeams ? "par équipes" : "en solo"}{" "}
                {tournament.bracketSettings[1].size ?
                  `(${tournament.bracketSettings[1].size} ${tournament.settings.useTeams ? "équipes" : "joueurs"})`
                : ""}
              </div>
            </div>
          </>
              ...
              tournament.bracketSettings[0].advancers && (
                <div className="fade-text ml-1">
                  ...
                  {GetFFAMaxPlayers(tournament.bracketSettings[0].sizes, tournament.bracketSettings[0].advancers) *
                    (tournament.settings.useTeams ? tournament.settings.teamsMaxSize || 1 : 1)}{" "}
                  max
                </div>
              )}
          </div>
        ...
        <div className="is-flex gap-3 wrap">
          ...
          <div>
            {Days[tournament.properties.startTime.day]} {tournament.properties.startTime.hour}h
          </div>
        </div>
        {/* Comments */}
        <div className="is-flex gap-3 wrap">
          {tournament.properties.comments && <div className="">Commentaires :</div>}
          {tournament.properties.comments && (
            <div className="is-scrollable enable-line-break">
              <FormattedTextWithUrls text={tournament.properties.comments} />
            </div>
          )}
        </div>
      </div>
