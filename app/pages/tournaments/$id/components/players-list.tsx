  ...
  return (
    <div className="is-flex-col grow gap-2">
      <DragOverlay style={{ opacity: ".75" }}>
        {draggingPlayer != null ? (
          <UserTileRectangle
            userId={draggingPlayer}
            colorClass={draggingPlayer == user.id ? "has-background-primary-accent" : "has-background-secondary-level"}
          />
        ) : null}
      </DragOverlay>
      <div className="is-title medium is-uppercase">Inscrits</div>
      <DndContext onDragStart={onDragStart} onDragEnd={onDragEnd} sensors={dndSensors}>
                  ...
                  <div key={player.userId} className="grow">
                    <Sortable id={player.userId}>
                      <PlayerTileWithCommands
                        ...
                        command={removeUserFromTournament}
                        commandSymbol={<BinSVG />}
                        seed={user.isAdmin && tournament.status == TournamentStatus.Balancing ? index + 1 : undefined}
                        isDraggable={user.isAdmin}
                      />
                    </Sortable>
                  </div>
                ))}
...
export function OpponentsListTeam() {
  ...
  const [sortableTeams, setSortableTeams] = useState<(Team & { id: string })[]>(
  ...
  const notInTeamPlayers = tournament.players
    .filter(
      (player) =>
        !(tournament.teams ? tournament.teams.flatMap((team) => team?.members) : ([] as string[])).includes(
          player.userId
        )
    )
    .map((player) => player.userId)
  const canAddTeam =
    tournament.bracketSettings[0].type != BracketType.FFA ||
    (tournament.bracketSettings[0].type == BracketType.FFA &&
      (tournament.teams || []).length <
        GetFFAMaxPlayers(tournament.bracketSettings[0].sizes || [], tournament.bracketSettings[0].advancers || []))
  ...
  async function onDragStart(event: DragStartEvent) {
    if ((event.active.id + "").includes("team_")) setDraggingTeam(event.active.id + "")
    else setDraggingPlayer(event.active.id + "")
    document.body.classList.add("is-dragging")
  }
    ...
    if (draggingPlayer) {
      ...
      if (over?.id && (over.id as string).startsWith("team_")) {
        ...
        if (active.data.current && over.id != active.data.current.team) {
          ...
          if (
            targetTeam &&
            tournament.settings.teamsMaxSize &&
            targetTeam.members.length < tournament.settings.teamsMaxSize
          ) {
  ...
  return (
    <DndContext onDragEnd={onDragEnd} onDragStart={onDragStart} sensors={dndSensors}>
      <DragOverlay style={{ opacity: ".75" }}>
        {draggingPlayer ? (
          <UserTileRectangle
            userId={draggingPlayer.replace("player_", "")}
            colorClass={
              draggingPlayer.replace("player_", "") == user.id
      ...
      <CustomModalBinary
              ...
              <div className="is-flex align-center gap-3">
                ...
                <div>
                  <input
                    ...
                    onKeyDown={(e) => e.key == "Enter" && newTeam()}
                  />
                </div>
              </div>
            </div>
        ...
        confirmCondition={() =>
          ...
          !(
            tournament.teams &&
            tournament.teams.map((team) => team.name.toLowerCase()).includes(newTeamName.toLowerCase())
          )
        }
        cantConfirmTooltip={
          newTeamName.toLowerCase() == "" ? "Le nom d'équipe ne peut pas être vide" : "Nom d'équipe déjà utilisé"
        }
      />
      <div className="is-flex-col grow gap-2">
        <div className="is-flex justify-space-between align-center">
          ...
          {(user.isAdmin ||
            (tournament.settings.usersCanCreateTeams &&
              notInTeamPlayers.includes(user.id) &&
              tournament.status == TournamentStatus.Open)) &&
            canAddTeam && (
              <CustomButton
                callback={() => setShowNewTeam(true)}
                contentItems={[SubsribedSVG(), "New team"]}
                customClasses="small-button"
...
export function TournamentInfoPlayers() {
  ...
  if (![TournamentStatus.Open, TournamentStatus.Balancing, TournamentStatus.Done].includes(tournament.status))
    return TournamentInfoPlayersWhileRunning()
  return null
}
function TournamentInfoPlayersWhileRunning() {
  ...
  tournament.matches
    .filter((m) => m.bracket == tournament.currentBracket)
  ...
  return (
    <div className="is-flex-col gap-2" style={{ height: 300 }}>
          ...
          return (
            <Fragment key={playerId}>
              <div className="is-flex-row gap-3">
                ...
                {user.isAdmin && (
                  <div className="grow is-flex justify-end">
                    <SquareButton
                      height={32}
                      contentItems={[hasFF ? RollBackSVG() : ForfeitSVG()]}
                      callback={() => toggleForfeit(playerId)}
                      colorClass={hasFF ? "has-background-secondary-level" : "has-background-secondary-accent"}
                    />
                  </div>
                )}
              </div>
...
function TournamentInfoPlayersOnceDone() {
  ...
  return (
    <div className="is-flex-col gap-2 no-basis" style={{ height: 300 }}>
        ...
        {tournament.results?.map((result, index) => (
          <Fragment key={result.userId}>
            <div className="is-flex-row gap-3 mr-2">
              <div className="has-text-right" style={{ width: 25 }}>
                {result.position}
              </div>
...
function TeamTile({ team, seed, draggedPlayer, addPlayerToTeam, removePlayerFromTeams }: TeamTileProps) {
  ...
  const isFull = !(
    ...
    tournament.settings.teamsMaxSize &&
    team.members.length < tournament.settings.teamsMaxSize
  )
  ...
  return (
    <div ref={setNodeRef} className="is-flex-col has-background-secondary-level p-2">
      <div className="is-flex-row align-center justify-space-between">
        <div className="is-uppercase grow no-basis" style={{ textOverflow: "ellipsis", overflow: "hidden" }}>
          {seed ? String(seed) + " - " : ""}
          {team.name}
        </div>
        {user.isAdmin && (
          <div className="is-flex-row align-center gap-1">
            <div className="is-clickable is-flex fade-on-mouse-out" {...clickorkey(() => setShowRenameTeam(true))}>
              <SubsribedSVG />
            </div>
            <div className="is-clickable is-flex fade-on-mouse-out" {...clickorkey(() => setShowDeleteTeam(true))}>
              <BinSVG />
            </div>
            <CustomModalBinary
              show={showRenameTeam}
                    ...
                    <div className="is-flex align-center gap-3">
                      ...
                      <div>
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter") renameTeam(team.name, teamName)
                          }}
                        />
                      </div>
                    </div>
              ...
              confirmCondition={() =>
                ...
                !(
                  tournament.teams &&
                  tournament.teams.map((team) => team.name.toLowerCase()).includes(teamName.toLowerCase())
                )
              }
            />
            <CustomModalBinary
      ...
      <div
        className="is-flex-col gap-1"
        style={{
          opacity:
            draggedPlayer && (isFull || team.members.includes(draggedPlayer.replace("player_", ""))) ? "50%" : "100%"
        }}
      >
            ...
            <div key={player}>
              {user.isAdmin ? (
                ...
                <UserTileRectangle
                  userId={player}
                  isShiny={index == 0}
                  colorClass={player == user.id ? "has-background-primary-accent" : undefined}
                />
              )}
            </div>
          ) : null
        ...
        {tournament.status == TournamentStatus.Open &&
          (team.members.includes(user.id) ? (
            <div
              className="is-flex justify-center align-center is-unselectable px-4 py-2 is-clickable has-background-secondary-accent"
              {...clickorkey(() => removePlayerFromTeams(user.id))}
            >
              <div className="">Quitter l&apos;équipe</div>
            </div>
          ) : !isFull && couldJoin ? (
            <div
              className="is-flex justify-center align-center is-unselectable px-4 py-2 is-clickable has-background-primary-accent"
              {...clickorkey(() => addPlayerToTeam(user.id, team.name))}
            >
...
function PlayerWithoutTeamArea() {
  ...
  const notInTeamPlayers = tournament.players
    .filter((player) => !([] as string[]).concat(...teams.map((team) => team.members)).includes(player.userId))
    .map((player) => player.userId)
  ...
  return (
    <>
        ...
        {user.isAdmin && (
          <div className="is-flex">
            {notInTeamPlayers.length > 0 ? (
              <CustomButton
                callback={distributePlayers}
                tooltip="Répartir les joueurs sans équipe"
                contentItems={[DistributeSVG(), "Distribuer"]}
                customClasses="small-button px-1 ml-3"
                colorClass="has-background-primary-level"
              />
            ) : (
              <CustomButton
                callback={balancePlayers}
                tooltip="Équilibrer les équipes"
                contentItems={[BalanceSVG(), "Équilibrer"]}
                customClasses="small-button px-1 ml-3"
                colorClass="has-background-primary-level"
              />
            )}
            <CustomButton
              callback={randomizePlayers}
              tooltip="Mélanger les joueurs dans des équipes"
              contentItems={[RandomSVG(), "Mélanger"]}
              customClasses="small-button px-2 ml-3"
              colorClass="has-background-primary-level"
            />
          </div>
        )}
      ...
      <div
        ref={setNodeRef}
        className="is-flex-col grow has-background-primary-level"
...
function PlayerTileWithCommands({
  userId,
  command,
  commandSymbol,
  isDraggable,
  ...
  seed,
  isShiny
}: PlayerTileWithCommandsProps) {
  const [hooveredPlayer, setHooveredPlayer] = useState(false)
  const user = useUser()

  return (
    <div
      className={`is-flex align-center grow ${isDraggable ? "is-draggable" : ""} ${
        userId == user.id ? "has-background-primary-accent" : baseColor ? baseColor : "has-background-secondary-level"
      }`}
      onMouseEnter={() => setHooveredPlayer(true)}
      onMouseLeave={() => setHooveredPlayer(false)}
    >
      <UserTileRectangle userId={userId} initial={seed ? String(seed) : undefined} isShiny={isShiny} />
      {user.isAdmin && (
        <div
          className="is-flex align-center is-clickable"
          style={{ width: "20px" }}
          {...clickorkey(() => command(userId))}
        >
