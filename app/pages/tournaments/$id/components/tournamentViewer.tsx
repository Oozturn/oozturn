...
export function TournamentViewer() {
  ...
  const [tournamentWideView, setTournamentWideView] = useLocalStorageState<string[]>("tournamentWideView", {
    defaultValue: []
  })
  ...
  useEffect(() => {
    ...
    if (OpponentInfosContainer.offsetHeight < OpponentInfos.offsetHeight) {
      ...
      OpponentInfos.style.setProperty(
        "--dist",
        String(OpponentInfosContainer.offsetHeight - OpponentInfos.offsetHeight) + "px"
      )
    }
  }, [hightlightOpponent])

  return (
    <div className="is-flex grow no-basis has-background-primary-level p-4 is-relative">
      <div
        style={{ position: "absolute", top: 0, left: 0, zIndex: 3 }}
        className="is-clickable is-flex p-2 fade-on-mouse-out"
        {...clickorkey(() =>
          setTournamentWideView(
            tournamentWideView.includes(tournament.id)
              ? tournamentWideView.filter((s) => s != tournament.id)
              : [...tournamentWideView, tournament.id]
          )
        )}
      >
        {tournamentWideView.includes(tournament.id) ? (
      ...
      {tournament.bracketsCount == 2 && (
        <div
          className="is-flex-row justify-center gap-3 p-2"
          style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 2 }}
        >
          <div
            className={currentBracketView == 0 ? "has-text-weight-bold" : "fade-on-mouse-out is-clickable"}
            {...clickorkey(() => setCurrentBracketView(0))}
          >
            Poules
          </div>
          <div>|</div>
          <div
            className={currentBracketView == 0 ? "fade-on-mouse-out is-clickable" : "has-text-weight-bold"}
            {...clickorkey(() => setCurrentBracketView(1))}
          >
            Finale
          </div>
        </div>
      ...
      <HightlightOpponentContext.Provider
        value={{ hightlightOpponent: hightlightOpponent, setHightlightOpponent: setHightlightOpponent }}
      >
        <div ref={containerRef} className="is-flex grow no-basis">
          <TransformWrapper
            centerOnInit={true}
            initialScale={minScale ? minScale : 1}
            minScale={minScale ? minScale : 0.3}
            maxScale={1}
          ...
          >
            ...
            >
              {width ? (
                <div ref={bracketRef}>
                  <BracketViewer
                    bracket={currentBracketView <= tournament.bracketsCount - 1 ? currentBracketView : 0}
                  />
                </div>
              ) : null}
            </TransformComponent>
        ...
        {tournament.settings.useTeams
              ...
              <div id="OpponentInfosContainer" className="OpponentInfosContainer has-background-primary-level">
                <div id="OpponentInfos" className="is-flex-col p-4 gap-4 is-relative">
                  ...
                  <div className="pl-2 is-flex-col gap-1">
                    {tournament.teams
                      .find((team) => team?.name == hightlightOpponent)
                      ?.members.map((player, index) => (
                        <UserTileRectangle
                          ...
                          maxLength={245}
                          showTeam={false}
                          isForfeit={tournament.players.find((p) => p.userId == player)?.isForfeit}
                        />
                      ))}
                  </div>
                </div>
          ...
          : hightlightOpponent &&
            tournament.players
              .filter((player) => player?.userId == hightlightOpponent)
              .map((player) => {
                ...
                return (
                  <div
                    key={user.id}
                    id="OpponentInfosContainer"
                    className="OpponentInfosContainer has-background-primary-level"
                  >
                    <div id="OpponentInfos" className="is-flex-col p-4 gap-4 is-relative">
                      ...
                      <div className="pl-2 is-flex-col gap-1">
                        <div className="is-flex align-center gap-2">
                          <GroupSVG />
                          {user.team ? user.team : "Solo"}
                        </div>
                        <div className="is-flex align-center gap-2">
                          <MapPinSVG /> {user.seat ? user.seat : "Not set"}
                        </div>
                      </div>
                    </div>
                  </div>
...
function GroupStageSectionViewer({ bracket, section }: { bracket: number; section: number }) {
  ...
  const getScore = ({ wins, draws }: { wins: number; draws?: number }) => {
    return (
      (tournament.bracketSettings[bracket].winPoints || 3) * wins +
      (tournament.bracketSettings[bracket].tiePoints || 1) * (draws || 0)
    )
  }
  ...
  return (
    <div className="is-flex-col gap-5 align-center">
      ...
      {results && (
        <div className="is-flex-row has-background-secondary-level gap-3 p-1 has-text-centered mb-5">
          <div className="is-flex-col gap-1" style={{ width: 245 }}>
            <div className="has-text-weight-semibold mb-2">Joueurs</div>
            {results.map((res) => (
              <div
                key={"id_" + res.id}
                onMouseEnter={() => setHightlightOpponent(res.id || "")}
                onMouseLeave={() => setHightlightOpponent("")}
              >
                {tournament.settings.useTeams ? (
                  <FakeUserTileRectangle
                    userName={res.id}
                    height={32}
                    initial={res.id[0]}
                    maxLength={245}
                    colorClass={getOpponentColorClass(res.id)}
                  />
                ) : (
                  <UserTileRectangle
                    userId={res.id}
...
function RoundViewer({ bracket, section, round }: { bracket: number; section: number; round: number }) {
  const matches = useTournament().matches.filter(
    (match) => match.bracket == bracket && !match.isFinale && match.id.s == section && match.id.r == round
  )
  ...
  return (
    <div className="is-flex-col gap-1 justify-center">
      <div className="is-flex-row gap-3 justify-space-around is-relative">
        <div className="pr-5 is-title medium" style={{ position: "absolute", left: "2rem", top: "-2.5rem" }}>
          Finale
        </div>
        {matches.map((match) => (
          <MatchTile key={bracket + "." + IdToString(match.id)} matchId={match.id} />
...
function MatchTile({ matchId }: { matchId: Id }) {
  ...
  const fetcherData = fetcher.data as
    | { error?: string; type?: MatchesIntents; matchID?: string; opponent?: string }
    | undefined
  ...
  const ffOpponentsIds = [
    ...
    ...tournament.teams.filter((t) => t.isForfeit).map((t) => t.name)
  ]
  const userTeam = tournament.teams.find((team) => team.members.includes(user.id))
  ...
  if (isOver) {
    ...
    else matchOpponents.sort((a, b) => b.opponentScore! - a.opponentScore!)
  }
  ...
  const isMatchEditable = matchOpponents.every((opponent) =>
    canEditScore(match, opponent.opponentId, tournament, user, settings.security.allOpponentsScore)
  )
  ...
  return (
    <div className="is-flex-row align-center" style={{ width: isFFA ? 394 : 330 }}>
      ...
      <div
        className={`is-flex-col ${
          match.isFinale ? "has-background-secondary-accent" : "has-background-secondary-level"
        } ${matchTileAccent} grow p-1 gap-1`}
      >
        {isFFA && (
        ...
        {matchOpponents.map(({ opponentId, opponentScore }, index) => {
          return (
            <div
              key={IdToString(matchId) + "-" + opponentId + "-" + String(index)}
              className="is-flex-row align-end justify-space-between gap-2"
              onMouseEnter={() => setHightlightOpponent(opponentId || "")}
              onMouseLeave={() => setHightlightOpponent("")}
            >
              {opponentId != undefined ? (
                tournament.settings.useTeams ? (
                  <FakeUserTileRectangle
                    userName={opponentId}
                    initial={opponentId[0]}
                    maxLength={245}
                    colorClass={getOpponentColorClass(opponentId)}
                  />
                ) : (
                  <UserTileRectangle
                    userId={opponentId}
              ...
              {canEditScore(match, opponentId, tournament, user, settings.security.allOpponentsScore) &&
              !ffOpponentsIds.includes(opponentId!) ? (
                <DebouncedInputNumber
                  name="score"
                  className={`threeDigitsWidth has-text-centered ${
                    isMatchOver ? "has-background-secondary-level" : ""
                  }`}
                  defaultValue={opponentScore}
                  setter={(v: number | undefined) => {
                    score(matchId, opponentId || "", v)
                  }}
                  debounceTimeout={3000}
                  error={errors.some((e) => e.matchID == IdToString(matchId) && e.opponent == opponentId)}
                />
              ) : (
                <div className="has-text-centered" style={{ width: "2.5rem" }}>
                  {ffOpponentsIds.includes(opponentId!) ? "F" : opponentScore != undefined ? opponentScore : ""}
                </div>
              )}
              {isFFA && (
                <div
                  className={`threeDigitsWidth has-text-centered ${
                    (
                      tournament.bracketsCount == 2 && match.bracket == 0
                        ? qualifiedPlayers.includes(opponentId || "")
                        : index < Math.floor(qualifiedPlaces)
                    )
                      ? "has-text-primary-accent"
                      : ""
                  }`}
  ...
  return (
    <div className="is-flex-row gap-2 align-end has-background-secondary-level has-text-centered">
      ...
      {matches.map((match, round) => {
        ...
        if (isReturn) mScore.reverse()
        return (
          <div key={IdToString(match.id)} className="is-flex-col p-1 gap-1">
            ...
            {mScore.map((opponentScore, index) => {
              ...
              return (
                <div
                  key={IdToString(match.id) + "-" + String(index)}
                  className="is-flex-row align-end justify-space-between gap-2"
                  onMouseEnter={() => setHightlightOpponent(opponentId || "")}
                  onMouseLeave={() => setHightlightOpponent("")}
                >
                  ...
                  !ffOpponentsIds.includes(opponentId!) ? (
                    ...
                    <div
                      className={`has-text-centered ${opponentScore != undefined ? "" : "fade-on-mouse-out"}`}
                      style={{ width: "2.5rem", height: 32 }}
                    >
                      {opponentScore != undefined ? (ffOpponentsIds.includes(opponentId!) ? "F" : opponentScore) : "?"}
                    </div>
                  )}
                </div>
              )
