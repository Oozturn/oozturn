...
export function TournamentViewer() {
    ...
    }, [containerRef, bracketRef, minScale, tournament.id, tournamentWideView])
        ...
        <div className="is-flex-row align-center" style={{ width: isFFA ? 339 : 275 }}>
            ...
            <div className={`is-flex-col ${match.isFinale ? 'has-background-secondary-accent' : 'has-background-secondary-level'} grow p-1 gap-1`}>
                ...
                {matchOpponents.map(({ opponentId, opponentScore }, index) => {
                    ...
                    return <div key={IdToString(matchId) + '-' + String(index) + '-' + String(opponentId)} className="is-flex-row align-end justify-space-between gap-2" onMouseEnter={() => setHightlightOpponent(opponentId || "")} onMouseLeave={() => setHightlightOpponent("")}>
