...
function BracketViewer({ bracket }: { bracket: number }) {
  ...
  return (
    <div
      className={`is-flex-${tournament.bracketSettings[bracket].type == BracketType.GroupStage ? "row" : "col"} gap-5 no-basis has-background-secondar-level`}
      style={{ margin: "2rem" }}
    >
...
function MatchTile({ matchId }: { matchId: Id }) {
  ...
  return (
    <div className="is-flex-row align-center" style={{ width: isFFA ? 394 : 330 }}>
      ...
      <div
        className={`is-flex-col ${match.isFinale ? "has-background-secondary-accent" : "has-background-secondary-level"} ${matchTileAccent} grow p-1 gap-1`}
      >
        {isFFA && (
