...
export function startTournament(tournamentId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to start tournament ${tournamentId}: ${
...
export function stopTournament(tournamentId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to stop tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function cancelTournament(tournamentId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to cancel tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function validateTournament(tournamentId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to validate tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function togglePauseTournament(tournamentId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to toggle pause for tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function toggleBalanceTournament(tournamentId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to toggle balance for tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function addPlayerToTournament(tournamentId: string, userId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to add player ${userId} to tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function toggleForfeitPlayerForTournament(tournamentId: string, userId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to forfeit player ${userId} for tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function removePlayerFromTournament(tournamentId: string, userId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to remove player ${userId} from tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function reorderPlayers(tournamentId: string, oldIndex: number, newIndex: number) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to reorder players in tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function reorderTeams(tournamentId: string, oldIndex: number, newIndex: number) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to reorder teams in tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function addTeamToTournament(tournamentId: string, teamName: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to add team ${teamName} to tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function addPlayerToTeam(tournamentId: string, teamName: string, userId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to add player ${userId} to team ${teamName}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function removePlayerFromTeams(tournamentId: string, userId: string) {
  try {
    ...
    logErrorAndThrow(
      `Error while trying to remove player ${userId} from teams: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function distributePlayersOnTeams(tournamentId: string) {
  ...
  try {
    ...
    logErrorAndThrow(
      `Error while trying to distribute players in tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
...
export function balanceTeams(tournamentId: string) {
  ...
  try {
    ...
    logErrorAndThrow(
      `Error while trying to balance teams in tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
  ...
  try {
    ...
    if (
      !canEditScore(
        ...
        user,
        process.env.ALL_OPPONENTS_SCORE === "duel_only"
          ? "duel_only"
          : process.env.ALL_OPPONENTS_SCORE === "true"
          ? true
          : false
      )
    )
    ...
    logErrorAndThrow(
      `Error while trying to score in match ${matchID} of tournament ${tournamentId}: ${
        error instanceof Error ? error.message : "Unknown Error"
      }`
    )
  }
