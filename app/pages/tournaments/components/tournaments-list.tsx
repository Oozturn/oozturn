    ...
    <div className="tournamentsList is-flex is-flex-direction-column is-scrollable">
      {user.isAdmin && (
        <NavLink
          to="/tournaments/new"
          className={({ isActive }) =>
            `tournamentTile has-background-secondary-level is-clickable ${isActive ? "is-active" : ""}`
          }
        >
      ...
      {tournaments.map((tournament) => (
        ...
        >
          ...
          {tournament.players.find((p) => p.userId == user.id) && (
            <div
              className="subscribed mini is-flex align-center has-background-primary-accent"
              style={{ zIndex: tournament.status == TournamentStatus.Done ? 1 : 2 }}
            >
              <SubsribedSVG />
            </div>
          )}
