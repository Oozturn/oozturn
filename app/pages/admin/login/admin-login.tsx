...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  return [{ title: data?.lanName + " - Connexion admin" }]
}
...
export async function loader({ request }: LoaderFunctionArgs): Promise<{
  ...
  return {
    lanName: getLan().name,
    adminLoginNoPassword: !process.env.ADMIN_PASSWORD || process.env.ADMIN_PASSWORD === "" ? true : false
  }
}
      ...
      <Form method="POST" className="p-4 has-background-secondary-level is-flex-col align-center gap-4 is-relative">
        ...
        <div className="is-flex align-center gap-2 has-background-primary-level">
          ...
          <input
            ...
            required
          />
          <div className="pr-2" onMouseEnter={() => setShowPassword(true)} onMouseLeave={() => setShowPassword(false)}>
            <EyeSVG />
          </div>
        </div>
        <button type="submit" className="customButton has-background-primary-accent is-clickable">
          Soumettre
        </button>
      </Form>
    </div>
