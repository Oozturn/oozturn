...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  return [{ title: data?.lanName + " - Accueil" }]
}
            ...
            <div className="is-scrollable grow">
              <div className="homeTournamentsGrid pr-4">
                {me?.isAdmin && (
                  <Link
                    to="/tournaments/new"
                    className="homeTournamentBox has-background-primary-accent is-flex-col justify-center align-center is-clickable fade-on-mouse-out"
                    data-id="editTournament"
                  >
  ...
  return (
    <Link to={`/tournaments/${tournament.id}`} className="homeTournamentBox is-clickable p-0">
      <img
        ...
        src={tournament.picture ? `/tournaments/${tournament.picture}` : "/none.webp"}
        style={{
          position: "absolute",
          objectFit: "cover",
      ...
      {tournament.status == TournamentStatus.Balancing && <div className="status-bar">En préparation</div>}
      {tournament.status == TournamentStatus.Paused && <div className="status-bar">En pause</div>}
      {tournament.status == TournamentStatus.Running && <div className="status-bar">En cours</div>}
      {tournament.status == TournamentStatus.Validating && <div className="status-bar">En validation</div>}
      {tournament.status == TournamentStatus.Done && <div className="over-overlay">Terminé</div>}
    </Link>
  )
}
