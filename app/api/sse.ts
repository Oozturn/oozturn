...
import {
  EVENT_SERVER_ERROR,
  EVENT_UPDATE_LAN,
  EVENT_UPDATE_TOURNAMENT,
  EVENT_UPDATE_TOURNAMENTS,
  EVENT_UPDATE_USERS,
  notificationProps,
  serverErrorEventProps,
  TOURNAMENT_UPDATE_TYPES,
  tournamentUpdateEventProps
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return eventStream(request.signal, function setup(send) {
    function handleUpdateTournament({ updateType, tournamentId }: tournamentUpdateEventProps) {
      ...
      if (
        ...
        if (updateType == TOURNAMENT_UPDATE_TYPES.START)
          send({
            event: SSE_NOTIFICATION_MESSAGE_EVENT,
            data: JSON.stringify(<notificationProps>{
              time: new Date().toISOString(),
              messageType: "startTournament",
              data: JSON.stringify({ id: tournamentId, name: tournament.getProperties().name })
            })
          })
        if (updateType == TOURNAMENT_UPDATE_TYPES.END)
          send({
            ...
            data: JSON.stringify(<notificationProps>{
              time: new Date().toISOString(),
              messageType: "endTournament",
              data: JSON.stringify({ id: tournamentId, name: tournament.getProperties().name })
            })
          })
      }
      send({
        event: EVENT_UPDATE_TOURNAMENT,
        data: JSON.stringify([new Date().toISOString(), updateType, tournamentId])
      })
    }
    ...
    function handleServerError({ userId: msgUserId, error }: serverErrorEventProps) {
      if (msgUserId != null && msgUserId != undefined && msgUserId != userId) return
      send({
        event: SSE_NOTIFICATION_MESSAGE_EVENT,
        data: JSON.stringify(<notificationProps>{ time: new Date().toISOString(), messageType: "error", data: error })
      })
    }
    ...
    emitter.on(EVENT_SERVER_ERROR, handleServerError)
    ...
    return function clear() {
      ...
      emitter.off(EVENT_SERVER_ERROR, handleServerError)
    }
  })
}
